...
public class EditProfileClientActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db.collection("users").document(userId).get().addOnSuccessListener(documentSnapshot -> {
            ...
            Toast.makeText(EditProfileClientActivity.this, "Error loading profile", Toast.LENGTH_SHORT).show();
        });
        ...
        db.collection("profile_descriptions").document(userId).get().addOnSuccessListener(documentSnapshot -> {
            ...
            Toast.makeText(EditProfileClientActivity.this, "Error loading descriptions", Toast.LENGTH_SHORT).show();
        });

        // Fetch and populate profile picture
        storageReference.child("profile_pictures/" + userId).getDownloadUrl().addOnSuccessListener(uri -> {
            Glide.with(EditProfileClientActivity.this).load(uri).into(profilePicture);
        }).addOnFailureListener(e -> {
            Toast.makeText(EditProfileClientActivity.this, "Error loading profile picture", Toast.LENGTH_SHORT).show();
        });
        ...
        saveButton.setOnClickListener(v -> {
            ...
            if (!firstName.isEmpty() || !middleName.isEmpty() || !lastName.isEmpty()) {
                ...
                db.collection("users").document(userId)
                        ...
                        .addOnFailureListener(e -> {
                            Toast.makeText(EditProfileClientActivity.this, "Error updating profile", Toast.LENGTH_SHORT).show();
                        });
            } else {
                profileUpdated = true;
                showToastUpdate();
            ...
            if (hasNonEmptyDescriptions) {
                ...
                db.collection("profile_descriptions").document(userId)
                        ...
                        .addOnFailureListener(e -> {
                            Toast.makeText(EditProfileClientActivity.this, "Error updating descriptions", Toast.LENGTH_SHORT).show();
                        });
            } else {
                descriptionsUpdated = true;
                showToastUpdate();
    ...
    private void uploadImageToFirebase(Bitmap bitmap) {
        ...
        profilePicRef.putBytes(data).addOnSuccessListener(taskSnapshot -> {
            profilePicRef.getDownloadUrl().addOnSuccessListener(uri -> {
                ProfilePictureManager.updateProfilePicture(EditProfileClientActivity.this, uri);
                Toast.makeText(EditProfileClientActivity.this, "Profile picture updated", Toast.LENGTH_SHORT).show();
                Glide.with(EditProfileClientActivity.this).load(uri).into(profilePicture);
            });
        }).addOnFailureListener(e -> {
            Toast.makeText(EditProfileClientActivity.this, "Error uploading profile picture", Toast.LENGTH_SHORT).show();
        });
    }
    private void showToastUpdate() {
        if (profileUpdated && descriptionsUpdated && pictureUpdated) {
            Toast.makeText(EditProfileClientActivity.this, "Profile, descriptions, and picture updated", Toast.LENGTH_SHORT).show();
            finish();
        } else if (profileUpdated && descriptionsUpdated) {
            Toast.makeText(EditProfileClientActivity.this, "Profile and descriptions updated", Toast.LENGTH_SHORT).show();
            finish();
        }
    }
